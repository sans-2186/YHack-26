...
# (category, weight, patterns) — patterns are lowercase substring or regex with (?i)
...
def primary_event_kind(tags: list[str]) -> str:
    """Map tag list to a coarse events[].kind bucket."""
    priority = (
        "antitrust",
        "sanctions",
        "regulation",
    ...
    for p in priority:
        if p in tags:
            return "regulation" if p == "legal" else p
    return "other"